...
function validDistro(d) { return /^[a-z0-9_-]+$/.test(d || ""); }
...
function createJob(distro, cmd) {
  ...
  // 后台任务独立进程组。cmd 仍作为单个 argv 交给容器内 bash -lc，避免宿主 shell 预解析。
  const wrapper = [
    "set +e",
  ...
  return { job_id: jobId, distro, status: "running", log: p.log, hint: "使用 job_status 查询；使用 job_output 获取输出；使用 job_cancel 取消" };
}
...
function readJob(jobId, includeOutput = false) {
  ...
  return { ...meta, status: running ? "running" : (result === 0 ? "completed" : "failed"), exit_code: result, log: p.log, ...(output === undefined ? {} : { output }) };
}
...
function cancelJob(jobId) {
  ...
  if (Number.isInteger(pid) && pid > 1) {
    // detached=true 后 pid 是独立进程组组长；负 PID 向整个任务组发信号，避免 proot/busy child 残留。
    try { process.kill(-pid, "SIGTERM"); } catch {}
  }
  return { ...readJob(jobId), status: "cancelling", hint: "稍后再次 job_status，确认任务已退出" };
}

function collectPids(rootPid, depth = 5) {
  let all = [], frontier = [rootPid];
  for (let i = 0; i < depth && frontier.length > 0; i++) {
    ...
    all.push(...children); frontier = children;
  }
  return all;
}
function killTree(rootPid) {
  for (const p of collectPids(rootPid).concat([rootPid]).reverse()) { try { process.kill(p, "SIGKILL"); } catch {} }
}
function execInContainer(distro, cmd, timeoutMs) {
  return new Promise((resolve, reject) => {
    ...
    let out = "", err = "";
    const timer = setTimeout(() => { killTree(child.pid); reject(new Error(`container exec 超时(${timeoutMs}ms)。长任务请使用 async=true。`)); }, timeoutMs);
    child.stdout.on("data", d => { out += d.toString(); });
    child.stderr.on("data", d => { err += d.toString(); });
    child.on("error", e => { clearTimeout(timer); reject(e); });
    child.on("close", code => { clearTimeout(timer); if (code !== 0) reject(new Error(err.trim() || out.trim() || `exit code ${code}`)); else resolve(out || err); });
  });
}
function convertPath(distro, targetPath, direction) {
  ...
  if (targetPath.startsWith(root)) { const rest = targetPath.slice(root.length); return rest === "" ? "/" : rest; }
