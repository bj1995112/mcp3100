...
// JSON Schema → zod raw shape。支持 enum / anyOf / oneOf / default，减少工具 schema 漏检。
function jsonSchemaToZod(s = {}) {
  if (s.anyOf || s.oneOf) {
    const variants = s.anyOf || s.oneOf;
    if (variants.length === 1) return jsonSchemaToZod(variants[0]);
    return z.union(variants.slice(0, 2).map((x) => jsonSchemaToZod(x)));
  }
  if (s.enum) {
    if (s.enum.length === 1) return z.literal(s.enum[0]);
    if (s.enum.every((v) => typeof v === "string")) return z.enum(s.enum);
    return z.union(s.enum.map((v) => z.literal(v)));
  }
  let zt;
  switch (s.type) {
    ...
    case "null": zt = z.null(); break;
    case "string":
    default: zt = z.string(); break;
  }
  if (s.description) zt = zt.describe(s.description);
  if (s.default !== undefined) zt = zt.default(s.default);
  return zt;
}
...
function createMcpServer() {
  const s = new McpServer({ name: "termux-mcp-std", version: "1.0.0" }, { capabilities: { tools: {} } });
  for (const tool of tools) {
    s.registerTool(tool.name, { description: tool.description || "", inputSchema: jsonSchemaToZodShape(tool.inputSchema) }, async (args) => {
...
const httpServer = http.createServer(async (nodeReq, nodeRes) => {
  try {
    if (nodeReq.method === "GET" && (nodeReq.url === "/.well-known/oauth-protected-resource/mcp" || nodeReq.url === "/.well-known/oauth-protected-resource")) {
      ...
      nodeRes.end(JSON.stringify({ resource: `http://127.0.0.1:${PORT}/mcp`, authorization_servers: [`http://127.0.0.1:${PORT}`] }));
      return;
    }
    if (nodeReq.method === "GET" && nodeReq.url === "/.well-known/oauth-authorization-server") {
      nodeRes.writeHead(200, { "Content-Type": "application/json" });
      nodeRes.end(JSON.stringify({ issuer: `http://127.0.0.1:${PORT}`, authorization_endpoint: `http://127.0.0.1:${PORT}/oauth/authorize`, token_endpoint: `http://127.0.0.1:${PORT}/oauth/token`, response_types_supported: ["code"], grant_types_supported: ["authorization_code"], token_endpoint_auth_methods_supported: ["none"], code_challenge_methods_supported: ["S256"] }));
      return;
    }
    ...
    if (nodeReq.method === "POST") {
      ...
      webReq = new Request(url, { method: "POST", headers: nodeReq.headers, body: buf.length ? new Uint8Array(buf) : undefined });
